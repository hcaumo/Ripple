...
export const handler: APIGatewayProxyHandlerV2 = async (event) => {
    ...
    const { walletAddress, amount } = JSON.parse(event.body);
    console.log("Parsed request body:", { walletAddress, amount });

    try {

        await mintTokens(walletAddress, amount)
...
async function mintTokens(walletAddress, amount) {
    ...
    const contractAddress = "0xF4707d4C4Fb32DCB9214331193dbc707e6b9f93F";
    const privateKey = "PRIVATEKEYHERE"; // The private key with minting permissions
    ...
    // Convert the amount to the smallest unit if needed
    const smallestUnitAmount = ethers.parseUnits(amount.toString(), 18);

    try {
        // Ensure the walletAddress is a valid Ethereum address
        if (!ethers.isAddress(walletAddress)) {
            throw new Error('Invalid wallet address');
        }

        // Call the mint function from the smart contract
        const tx = await contract.mint(walletAddress, smallestUnitAmount);
        await tx.wait();
        console.log(`Minted ${amount} tokens to ${walletAddress}`);
    } catch (error) {
