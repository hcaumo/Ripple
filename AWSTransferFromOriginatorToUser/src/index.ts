...
export const handler: APIGatewayProxyHandlerV2 = async (event) => {
    ...
    const { contractAddress, fromAddress, toAddress, amount } = JSON.parse(event.body);
    console.log("Parsed request body:", { contractAddress, fromAddress, toAddress, amount });

    try {

        await transferTokens(contractAddress, fromAddress, toAddress, amount)
...
async function transferTokens(contractAddress, fromAddress, toAddress, amount) {
    const ABI = [
        {
            "inputs": [],
    ...
    const senderPrivateKey = await getPrivateKey(fromAddress)

    // Connect to the opBNB chain (or relevant network)
    ...
    // Convert the amount to the smallest unit if needed (assuming 18 decimal places)
    const smallestUnitAmount = ethers.parseUnits(amount.toString(), 18);

    try {
        // Execute the transferFrom function
        const tx = await contract.transferFrom(fromAddress, toAddress, smallestUnitAmount);
        await tx.wait();
